...
export default function ApiKeySetup({ onSave }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Test the key with a quick search before saving
    try {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 w-full max-w-md p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500">
            AcademicPath uses SerpAPI to search Google Scholar. Your key is stored only in your browser — never sent anywhere else.
          </p>
        </div>
